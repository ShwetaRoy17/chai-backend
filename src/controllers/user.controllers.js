...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  if (username?.trim()) {
    throw new ApiError(401, "UserProfile url not provided.");
  }
  ...
  const channel = await User.aggregate([
    ...
    },
    {
      $addFields:{
        ...
        isSubscribed:{
          $cond:{
            if:{$in:[req.user?._id,"$susbscribers.subscriber"]},
            then:true,
            else:false
          }
        }
...
const getWatchHistory = asyncHandler(async(req,res)=>{
 ...
 return res.status(200)
 .json(new ApiResponse(201,user[0].watchHistory,"User watched history fetched successfully."))
})
